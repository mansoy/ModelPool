...
uses System.IniFiles, System.StrUtils, Vcl.Imaging.GIFImg, Vcl.Imaging.JPEG,
     System.Variants, System.Win.ComObj, ManSoy.MsgBox, ManSoy.StrSub,
     ManSoy.Global, DmUtils;
...
begin
  ...
  FCheckImgName := GAppPath+'\CheckCode.jpg';
  FDmUtils := TDmUtils.Create;

  with TIniFile.Create(GAppPath+'\QuitSafeConfig\Cfg.ini') do
  try
...
begin
  result := false;
  tokenSrc_dir     := Format(TOKEN_SRC_DIR,  [GAppPath, FAccount]);
  tokenSrcZip_file := Format(TOKEN_SRC_ZIP_FILE,  [GAppPath, FAccount]);

  tokenDir_old     := Format(TOKEN_DIR_OLD,     [GAppPath]);
  kemulatorDir_old := Format(KEMULATOR_DIR_OLD, [GAppPath]);

  tokenDir_new     := Format(TOKEN_DIR_NEW,     [GAppPath]);
  kemulatorDir_new := Format(KEMULATOR_DIR_NEW, [GAppPath]);
  case FMBType of
    40: //KM(new)
    begin
  ...
  try
    case AMBType of
      ...
      begin
        ...
        ShellExecute(0, nil, PWideChar(kemulatorDir_new+'\'+KEMULATOR_BAT), nil, PWideChar(kemulatorDir_new), SW_HIDE);
        ...
        begin
          ...
          //pt := FDmUtils.FindWordOnWindow(hKEmulator, '动态密码', 'ffffff-000000', SYS_DICT);
        ...
        s := '';
        subPic := GAppPath + 'Bmp\%d.bmp';
        //第1位
        for I := 0 to 9 do
        begin
          if DmUtils.fnPicExistOnWindowArea(hKEmulator, 49,128,75,177, Format(subPic, [I])) then
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 49,128,75,177, Format(subPic, [I])) then
        ...
        begin
          ...
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 74,128,100,177,Format(subPic, [I])) then
          begin  s := s + IntToStr(I); Break; end;
        end;
        //第3位
        for I := 0 to 9 do
        begin
          ...
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 99,128,125,177,Format(subPic, [I])) then
          begin  s := s + IntToStr(I); Break; end;
        end;
        //第4位
        for I := 0 to 9 do
        begin
          ...
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 124,128,150,177,Format(subPic, [I])) then
          begin  s := s + IntToStr(I); Break; end;
        end;
        //第5位
        for I := 0 to 9 do
        begin
          ...
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 149,128,175,177,Format(subPic, [I])) then
          begin  s := s + IntToStr(I); Break; end;
        end;
        //第6位
        for I := 0 to 9 do
        begin
          ...
          //if FDmUtils.FindPicExistOnWindowArea(hKEmulator, 174,128,200,177,Format(subPic, [I])) then
          begin  s := s + IntToStr(I); Break; end;
        end;
      ...
      begin
        ManSoy.Global.DebugInf('打开手机令牌模拟器...', []);
        ShellExecute(0, nil, PWideChar(kemulatorDir_old+'\'+KEMULATOR_BAT), nil, PWideChar(kemulatorDir_old), SW_HIDE);
        dwDealyTimes := GetTickCount;
        while GetTickCount - dwDealyTimes < 1 * 60 * 1000 do
        begin
          hKEmulator := FindWindow(nil, 'KEmulator Lite v0.9.8');
        ...
        //TokenPackData.szDnyPsw := FDmUtils.FindWordByColor(hKEmulator, 50, 60, 200, 110, 'ffffff-000000', DIGIT_DICT, 0.8, 10);
